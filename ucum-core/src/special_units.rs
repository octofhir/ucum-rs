...
impl SpecialUnitHandler for TemperatureHandler {
    ...
    fn convert_from(
        ...
        match unit_code {
            ...
            _ => Err(UcumError::conversion_error("temperature unit", "conversion", "Unknown temperature unit")),
        }
    }
...
impl SpecialUnitHandler for LogarithmicHandler {
    ...
    fn can_handle(&self, unit_code: &str) -> bool {
        matches!(
            unit_code,
            "B" | "dB" | "Np" | "pH" | "pOH" | "pKa" | "pKw" | "pK" | "ln" | "log" | "log2" |
    ...
    fn convert_from(
        ...
        match unit_code {
            ...
            _ => Err(UcumError::conversion_error("logarithmic unit", "conversion", "Unknown logarithmic unit")),
        }
    }
...
mod tests {
    ...
    fn test_specialized_bel_units() {
        ...
        let spl_0db = handler.convert_from(from_f64(0.0), "B[SPL]", &context).unwrap();
        ...
        let spl_60db = handler.convert_from(from_f64(60.0), "B[SPL]", &context).unwrap();
        ...
        println!("Correct manual calc: {} Pa", manual_calc);
        ...
        let v_0db = handler.convert_from(from_f64(0.0), "B[V]", &context).unwrap();
        assert!((v_0db.to_f64() - 1.0).abs() < 1e-10);

        // 20 dB V should be 1 V * 10^(20/20) = 10 V
        let v_20db = handler.convert_from(from_f64(20.0), "B[V]", &context).unwrap();
        ...
        let mv_0db = handler.convert_from(from_f64(0.0), "B[mV]", &context).unwrap();
        assert!((mv_0db.to_f64() - 1e-3).abs() < 1e-10);

        // Test B[W] (power level)
        // 0 dB W = 1 W reference
        let w_0db = handler.convert_from(from_f64(0.0), "B[W]", &context).unwrap();
        assert!((w_0db.to_f64() - 1.0).abs() < 1e-10);

        // 10 dB W should be 1 W * 10^(10/10) = 10 W
        let w_10db = handler.convert_from(from_f64(10.0), "B[W]", &context).unwrap();
    ...
    fn test_specialized_bel_units_conversion_to() {
        ...
        let pa_to_spl = handler.convert_to(from_f64(0.02), "B[SPL]", &context).unwrap();
        ...
        let v_to_dbv = handler.convert_to(from_f64(10.0), "B[V]", &context).unwrap();
        ...
        let w_to_dbw = handler.convert_to(from_f64(10.0), "B[W]", &context).unwrap();
