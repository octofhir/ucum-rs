...
impl EvaluationCache {
    ...
    pub fn get_or_compute_expression<F>(&mut self, expr: &UnitExpr, compute: F) -> Result<EvalResult, UcumError>
    where
    ...
    pub fn get_or_compute_conversion<F>(&mut self, from_unit: &str, to_unit: &str, compute: F) -> Result<f64, UcumError>
    where
...
fn hash_unit_expr<H: Hasher>(expr: &UnitExpr, hasher: &mut H) {
    match expr {
        ...
        UnitExpr::Product(factors) => {
            ...
            let mut factor_hashes: Vec<_> = factors.iter().map(|f| {
