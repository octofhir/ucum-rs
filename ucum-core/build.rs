...
fn main() {
    ...
    for (code, val, exp, name) in &prefixes {
        out.push_str(&format!(
            "    Prefix {{ symbol: \"{}\", factor: {}f64, exponent: {}, display_name: \"{}\" }},\n",
            code, val, exp, name
        ));
    }
    out.push_str("];\n\n");
    ...
    for i in 0..units.len() {
        let needs_update = {
            let (_, dim, _, _, _, _, _, unit_ref) = &units[i];
        ...
        if needs_update {
            ...
            let derived_dim = if property_dim != [0i8; 7] {
                ...
                if ref_dim != [0i8; 7] {
                    ...
                    let fallback_property_dim = match units[i].5.as_str() {
                        "length" => [0, 1, 0, 0, 0, 0, 0],
                        "mass" => [1, 0, 0, 0, 0, 0, 0],
                        "time" => [0, 0, 1, 0, 0, 0, 0],
                        "electric current" => [0, 0, 0, 1, 0, 0, 0],
                ...
                if parsed_dim != [0i8; 7] {
                    ...
                    let base_dim = match ref_unit.as_str() {
                        "m" => [0, 1, 0, 0, 0, 0, 0],                   // length
                        "g" => [1, 0, 0, 0, 0, 0, 0],                   // mass
                        "s" => [0, 0, 1, 0, 0, 0, 0],                   // time
                        "A" => [0, 0, 0, 1, 0, 0, 0],                   // current
