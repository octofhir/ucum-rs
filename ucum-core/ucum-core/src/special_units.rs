...
use crate::types::{Dimension, SpecialKind};
...
pub trait SpecialUnitHandler: Send + Sync {
    ...
    fn convert_from(&self, value: Number, unit_code: &str, context: &ConversionContext) -> Result<Number, UcumError>;
    ...
    fn convert_to(&self, value: Number, unit_code: &str, context: &ConversionContext) -> Result<Number, UcumError>;
...
impl SpecialUnitHandler for TemperatureHandler {
    ...
    fn convert_from(&self, value: Number, unit_code: &str, _context: &ConversionContext) -> Result<Number, UcumError> {
        match unit_code {
            "Cel" => {
    ...
    fn convert_to(&self, value: Number, unit_code: &str, _context: &ConversionContext) -> Result<Number, UcumError> {
        match unit_code {
            "Cel" => {
...
impl SpecialUnitHandler for LogarithmicHandler {
    ...
    fn convert_to(&self, value: Number, unit_code: &str, _context: &ConversionContext) -> Result<Number, UcumError> {
        ...
        if val <= 0.0 {
            return Err(UcumError::ConversionError("Cannot take logarithm of non-positive value"));
        }
...
impl SpecialUnitHandler for ArbitraryHandler {
    ...
    fn convert_from(&self, value: Number, _unit_code: &str, _context: &ConversionContext) -> Result<Number, UcumError> {
        // Arbitrary units are typically dimensionless with factor 1
        Ok(value)
    }

    fn convert_to(&self, value: Number, _unit_code: &str, _context: &ConversionContext) -> Result<Number, UcumError> {
        // Arbitrary units are typically dimensionless with factor 1
        Ok(value)
    }
...
mod tests {
    ...
    fn test_temperature_handler() {
        ...
        let celsius_to_kelvin = handler.convert_from(from_f64(0.0), "Cel", &context).unwrap();
        assert!((celsius_to_kelvin.to_f64() - 273.15).abs() < 1e-10);

        // Test Kelvin to Celsius
        let kelvin_to_celsius = handler.convert_to(from_f64(273.15), "Cel", &context).unwrap();
        assert!(kelvin_to_celsius.to_f64().abs() < 1e-10);

        // Test Fahrenheit to Kelvin
        let fahrenheit_to_kelvin = handler.convert_from(from_f64(32.0), "[degF]", &context).unwrap();
    ...
    fn test_logarithmic_handler() {
        ...
        let db_to_ratio = handler.convert_from(from_f64(20.0), "dB", &context).unwrap();
