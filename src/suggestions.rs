...
impl SuggestionEngine {
        ...
        for unit in units {
            ...
            for other in units {
                if unit.code != other.code {
                    ...
                    if similarity > 0.7 {
                        // 70% similarity threshold for display names
                        similar.push(other.code.to_string());
                    }
                }
            }
    ...
    pub fn suggest_corrections(&self, invalid_unit: &str) -> Vec<String> {
        ...
        for unit in &self.all_units {
            ...
            let display_similarity = Self::string_similarity(
                &unit.display_name.to_lowercase(),
                &invalid_unit.to_lowercase(),
            );
            if display_similarity > 0.6 {
                // 60% similarity for display names
                scored_suggestions.push((
                    display_similarity,
                    unit.code.to_string(),
                    unit.display_name.to_string(),
    ...
    pub fn suggest_alternatives(&self, unit: &str, property: &str) -> Vec<String> {
        ...
        for other_unit in &self.all_units {
            if other_unit.property.to_lowercase() == property.to_lowercase()
                && other_unit.code != unit
            {
    ...
    fn suggest_prefixed_alternatives(&self, invalid_unit: &str, suggestions: &mut Vec<String>) {
        ...
        for unit in &self.all_units {
            ...
            if invalid_unit.len() > unit.code.len() && invalid_unit.ends_with(&unit.code) {
                ...
                if potential_prefix.len() <= 3 {
                    // Reasonable prefix length
                    suggestions.push(format!(
                        "Did you mean a prefixed version of '{}'? (e.g., 'k{}', 'm{}', 'u{}')",
                ...
                *first = first.to_uppercase().next().unwrap_or(*first);
                ...
                for unit in &self.all_units {
                    if unit.code == capitalized {
                        suggestions.push(format!("Did you mean '{capitalized}'? (capitalization)"));
                        break;
                    }
                }
            }
    ...
    pub fn suggest_dimension_fixes(
        &self,
        ...
        for unit in &self.all_units {
            if unit.property.to_lowercase() == expected_property.to_lowercase() {
                ...
                if similarity > 0.3 {
                    suggestions.push(format!(
                        "Use '{}' ({}) for {}",
                        unit.code, unit.display_name, expected_property
                    ));
                }
            }
        }
