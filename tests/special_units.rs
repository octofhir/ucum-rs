use octofhir_ucum::precision::{NumericOps, from_f64, to_f64};
use octofhir_ucum::{EvalResult, OwnedUnitExpr, OwnedUnitFactor, evaluate_owned};
...
fn test_prism_diopter_variations() {
    ...
    for (pd, expected) in test_cases.iter() {
        ...
        assert!(
            (r - expected).abs() < 1e-6,
            "{pd} [p'diop]: expected {expected}, got {r}"
        );
    }
}
...
fn test_special_unit_prefixes() {
    ...
    for (unit, value, expected) in test_cases.iter() {
        ...
        assert!(
            (r - expected).abs() < 1e-6,
            "{value} {unit}: expected {expected}, got {r}"
        );
    }
}
...
fn test_zero_special_units() {
    ...
    for (unit, expected) in test_cases.iter() {
        ...
        assert!(
            (r - expected).abs() < 1e-12,
            "0 {unit}: expected {expected}, got {r}"
        );
    }
}
