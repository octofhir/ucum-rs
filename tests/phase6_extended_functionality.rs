...
use octofhir_ucum::{
    Domain, MeasurementContext, analyse, canonicalize_expression, optimize_expression,
    simplify_expression,
};
...
fn test_measurement_context_suggest_alternatives() {
    ...
    assert!(
        pressure_alternatives.contains(&"Pa".to_string())
    ...
    assert!(
        medical_context
            .suggest_alternatives("invalid_unit")
            .is_err()
    );
}
...
fn test_measurement_context_domain_specific_suggestions() {
    ...
    for (context, expected_domain) in contexts {
        ...
        if let Ok(mass_suggestions) = context.suggest_alternatives("g") {
            assert!(
                !mass_suggestions.is_empty(),
                "Context {expected_domain:?} should provide mass unit suggestions"
            );
        }
    }
}
...
fn test_measurement_context_precision_requirements() {
    ...
    for context in contexts {
        ...
        assert!(
            context.precision_requirements.min_significant_figures >= 3,
            "Context {:?} should require at least 3 significant figures",
            context.domain
        );
        assert!(
            context.precision_requirements.max_relative_error > 0.0,
            "Context {:?} should have positive max relative error",
            context.domain
        );
        assert!(
            context.precision_requirements.max_relative_error < 1.0,
            "Context {:?} should have max relative error < 1.0",
            context.domain
        );
    }
}
